...
class FireRedTTS3BaseCore(PreTrainedModel):
    ...
    def generate(
        ...
        routing = getattr(self, "_lora_routing", ROUTING_GLOBAL)
        ...
        if routed:
            prompt_patches = prompt_latents.shape[1] // self.patch_size
            prompt_route = prompt_latents.new_zeros(prompt_patches, 1, 1)
            set_lora_route_mask(self.patch_encoder, prompt_route)
        else:
            set_lora_route_mask(self.patch_encoder, None)
            ...
            if routed:
                if step_index == 0:
                    backbone_route = build_backbone_route_mask(
                        ...
                        prompt_patches=patch_prompt_latents.shape[1],
                        target_patches=0,
                        reference=input_embeds,
                    )
                else:
                    # Subsequent calls contain exactly one generated target
                    # token; both routed policies enable its residual path.
                    backbone_route = input_embeds.new_ones(
                        (1, input_embeds.shape[1], 1)
                    )
                set_lora_route_mask(self.backbone_llm, backbone_route)
            else:
                set_lora_route_mask(self.backbone_llm, None)
